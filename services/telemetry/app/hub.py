...
from .models import Frame, FrameType
...
class TelemetryHub:
    ...
    async def start(self) -> None:
        if self._tasks:
            return
        self._tasks = [
            asyncio.create_task(self._market_loop(), name="telemetry-market"),
            asyncio.create_task(self._swarm_loop(), name="telemetry-swarm"),
            asyncio.create_task(self._activity_loop(), name="telemetry-activity"),
        ]
    ...
    async def _swarm_loop(self) -> None:
        while True:
            try:
                ...
                self.broadcast(self.frame(FrameType.SWARM, self.state.swarm().model_dump(mode="json")))
